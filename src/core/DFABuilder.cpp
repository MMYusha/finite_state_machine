...
void DFABuilder::readAlphabet(ifstream& file) {
    ...
    if (!getline(file, Line)) {
        throw std::runtime_error(
            "Не удалось прочитать строку с Алфавитом (шапка таблицы переходов)");
    }
        ...
        while (getline(ss, token, ';')) {
            ...
            // Чтение и запись таблицы переходов
                ...
                if (count_alphabet < alphabet.size()) {
                    // если токен пустой, то перехода по символу алфавита под номером count_alphabet
                    // не существует
                    if (!token.empty()) {
                ...
                // если count_alphabet превышает размер алфавита, то достигнут конец содержимого
                // строки
                else
                    break;
            }
...
// ------------------------------------------------------------
// Генератор ДКА для бенчмарка
// ------------------------------------------------------------

DFABuilder& DFABuilder::generatedDFA(int number_of_states, int alphabet_size, const string& mode,
                                     int seed) {
    // Проверки
...
void DFABuilder::generateStates(int number_of_states) {
    for (int i = 0; i < number_of_states; ++i) states.push_back("q" + to_string(i));
}

void DFABuilder::generateAlphabet(int alphabet_size) {
    for (int i = 0; i < alphabet_size; ++i) alphabet.push_back("q" + to_string(i));
}
    ...
    for (const auto& state : states) {
        for (const auto& sym : alphabet) {
            ...
            if (it_state != transitions.end() &&
                it_state->second.find(sym) == it_state->second.end()) {
...
void DFABuilder::addTransitionsToCapacity(vector<pair<string, string>> available_keys, int capacity,
                                          int seed) {
    mt19937 rng(seed);                      // Генератор случайных чисел с фиксированным seed
...
DFABuilder& DFABuilder::withComponents(
    string start_state, vector<string> states, vector<string> permitted_states,
    vector<string> alphabet, unordered_map<string, unordered_map<string, string>> transitions) {
