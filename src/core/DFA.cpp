...
DFA5::DFA5(const string& start, const vector<string>& st, const vector<string>& perm,
           const vector<string>& alph,
           const unordered_map<string, unordered_map<string, string>>& trans,
           const vector<string>& str_transition, const string current)
    : start_state(start),
      states(st),
      permitted_states(perm),
      alphabet(alph),
      transitions(trans),
      string_transition(str_transition),
...
const unordered_map<string, unordered_map<string, string>>& DFA5::getTransitions() const {
    return transitions;
};

void DFA5::resetCurrentState() {
    this->current_state = start_state;
};  // возвращение ДКА в начальное состояние
...
void DFA5::printStartState() { cout << "Начальное состояние - " << start_state << endl; }
...
void DFA5::printCurrentState() { cout << "Текущее состояние - " << current_state << endl; }
...
void DFA5::exportCSV(const string& filename) {
    ...
    exportAlphabetCSV(
        file);  // 4) Строка "состояния/алфавит;1;2;3;..." - шапка таблицы переходов, она же алфавит
    exportTransitionsCSV(file);  // 5) Строки состояний и переходов: первый столбец - набор
                                 // состояний; остальные столбцы - содержимое таблицы переходов
...
void DFA5::exportTransitionsCSV(ofstream& file) {
    // 5) Строки состояний и переходов: первый столбец - набор состояний; остальные столбцы -
    // содержимое таблицы переходов
    for (const auto& state : states) {
        file << state << ";";
